...
to the Mega Drive H40 raster.  H40 dots are not square, so scaling a source
directly to `320x224` stretches it.

The helper uses the hardware dot ratio and fits the complete source to the
...
| raster | HAR/PAR | visible DAR at 224 lines |
| ---: | ---: | ---: |
| 320x224 | 32:35 | 64:49 (about 1.306) |
...
places a 320x202 image inside the target raster.  The resulting top/bottom
border is only 11 lines.  The 522x400 centred crop remains available with
`--fit crop`, but it must not be used when the

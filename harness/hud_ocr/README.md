...
The common order is `F/P/S/D/R/L/C/W/M/A/U/N/J`; extended H40 appends
`Q/V/O/E`. `F/U/Q` contain four hexadecimal digits. `L` is the high byte of the audio lead;
...
patterns (`0000` empty, `FFFF` one-pattern underflow). The player formats these
...
four-digit `Q`, and confirms that

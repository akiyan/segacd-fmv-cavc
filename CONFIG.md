...
| `SUB_POLL_GAP_DIAG` | 0 | Experimental H40 DEBUG only: keep the complete `Q/V/O/E` first row and append `G/K` on row 1 to measure the longest interval outside the Sub CDC pump, APPLY back-pressure, and MSF-gap recovery cause. Release and ordinary DEBUG players are unchanged. |
...
| `SUB_POLL_GAP_DIAG` | 0 | 実験用H40 DEBUG専用。完全な`Q/V/O/E`の1行目を維持し、row 1へ`G/K`を追加して、Sub CDC pump外の最長interval、APPLY back-pressure、MSF-gap recovery原因を測る。Releaseと通常DEBUG playerは変わらない。 |

...
```
   ...
   every HUD value available in the TSV (`P/S/D/R/L/C/W/M/A/U/N/J/Q/V/O/E`).
...
- Put `VBLANK` first. Derive it from the difference between consecutive
  ...
  is specified.
- Include the values-only HUD fields `S/D/R/L/C/W/M/A/U/N/J` and `Q/V/O/E` when
...
- Follow with the remaining diagnostic, player-state, Sub, Main, and phase
  ...
  balance and positive underflow debt. Preserve HUD units instead of normalizing each recording to its
...
- Preserve the established per-metric colors for normal playback values:
  C yellow, M orange, J, Q, and W purple, L blue, A pink, U cyan, N orange, and

/*
 * Phase A: H40 256x144 静止画レンダラ (Main/IP)。
 *
 * 実機描画土台の切り分け用。CD読み込み/Sub/DMA/差分を一切使わず、IPに埋め込んだ
 * 1フレーム分(576タイル+ネームテーブル+パレット)を CPU書き込みだけで VRAM へ載せ、
 * 256x144(32x18) を H40画面の中央(縦offset 5セル/横offset 4セル)に表示して停止する。
 *
 * still256.bin レイアウト:
...
ip_entry:
	...
	/* --- VDP: H40, plane A=0xC000, 64x32, autoinc=2 --- */
	move.w	#0x8C81, (VDP_CTRL).l		/* reg12 = 0x81 : H40(320)   */
...
tile_loop:
	...
	/* --- nametable -> plane rows 5..22, cols 4..35 --- */
	lea	NT_OFF(a1), a0			/* a0 = nametable entries (cell順) */
	moveq	#5, d3				/* plane_row = 5 (縦中央: 28行に18行) */
	move.w	#18-1, d2			/* 18 rows */
row_loop:
	/* VRAM addr = NAME_A + (plane_row*64 + plane_col)*2, plane_col = 4 */
	move.l	d3, d0
	lsl.l	#7, d0				/* plane_row*128 */
	addq.l	#8, d0				/* plane_col 4 (横中央: 40列に32列) */
	add.l	#NAME_A, d0

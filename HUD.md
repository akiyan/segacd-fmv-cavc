...
| `N` | 26-27 | x=208-223 | 2 |
| `J` | 28-29 | x=224-239 | 2 |
| `Q` (H40 only) | 30-33 | x=240-271 | 4 |
| `V` (H40 only) | 34-35 | x=272-287 | 2 |
...
`N`, and `Q` describe one frame. `F`, `P`, and `L` describe current player state.
...
2. formats the HUD into a 60-byte Main-RAM row (80 bytes with `Q`/`V`/`O`/`E`);
...
15 longword writes (20 with `Q`/`V`/`O`/`E`) and no DMA. Unoccupied H32 cells
...
row uses `HUD_H40_FLIP_LAYOUT`. Pass `--flip-fields` to the recording analyzer
for an H40 DEBUG build.
...
32-cell rowにはextensionの余地がありません。
...
| `N` | 26-27 | x=208-223 | 2 |
| `J` | 28-29 | x=224-239 | 2 |
| `Q`（H40のみ） | 30-33 | x=240-271 | 4 |
| `V`（H40のみ） | 34-35 | x=272-287 | 2 |
...
`C/W/M/A/U/N/Q`は1 frame、`F/P/L`はcurrent stateです。
...
2. 60-byte Main-RAM row（`Q/V/O/E`付きは80 byte）へHUDをformat
...
`Q/V/O/E`付きで20 write、DMAなしです。H32の未使用cellはmovie entryを保持し、無関係な
...
共通H32/H40 prefixは`HUD_LAYOUT`、extended H40 rowは`HUD_H40_FLIP_LAYOUT`を使います。
H40 DEBUG buildのrecording analyzerには`--flip-fields`を渡します。
...
`prgbuf_underflow_patterns`へ書きます。
